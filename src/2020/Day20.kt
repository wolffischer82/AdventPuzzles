    ...
    fun fillImage() : ImageMesh?{
        for (y in 0..dimensions-1){
            for (x in 0..dimensions-1) {
                ...
                var candidates : List<TileOrientation>? = TileOrientation.leftEdgesMap.getOrElse(pixel!!.edges[1], {
                        listOf<TileOrientation>()})

                if (lineSwitch){
                    val edge = pixel!!.edges[2]
                    candidates = TileOrientation.topEdgesMap.getOrElse(edge, { listOf<TileOrientation>()})
                }
...
fun main(args : Array<String>){
    Day20("data/2020/Day20Input").run { part1() }
}